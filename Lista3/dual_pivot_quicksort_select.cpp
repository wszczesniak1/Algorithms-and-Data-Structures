...
int partition(int* arr, int p int q, int* lp);
...
void DualPivotQuickSort(int* arr, int p int q, int n)
{
    if (low < q) {
        amount_of_comparisons++; 
        int lp, rp;
        rp = partition(arr, p q, &lp);
        DualPivotQuickSort(arr, p lp - 1,n);
        DualPivotQuickSort(arr, lp + 1, rp - 1,n);
        DualPivotQuickSort(arr, rp + 1, q,n);
        if (n < 50){
...
int partition(int* arr, int p int q, int* lp)
{
    ...
    int pivot_index1 = select_pivot(arr, p q, 100, 5);
    swap(&arr[low], &arr[pivot_index1]);
    int pivot_index2 = select_pivot(arr, low+1, q, 100, 5);
    swap(&arr[q], &arr[pivot_index2]);
    ...
    int j = low + 1;
    int g = q - 1, k = low + 1, p = arr[low], q = arr[q];
    while (k <= g) {
        amount_of_comparisons++;
...
int main(){
    ...
    std::cout << "Liczba porównań " << amount_of_comparisons  << " " << " Liczba swapow:" << amount_of_displacements<< std::endl;
}